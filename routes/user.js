...
function testUser(){
    router.get('/:id', function(req, res) {
        ...
        (async () => {
            try {
                const posts = await TikTokScraper.user(id, { number: 50 });
                console.log(posts['collector']);


                        const getUserProfileInfo = await TikTokScraper.getUserProfileInfo(id, options);
                        console.log(getUserProfileInfo);

                        res.render('user',{
                            url:posts['collector'],
                            url2:getUserProfileInfo
                        })
...
function getVideo() {
    router.get('/:id', function (req, res) {
        ...
        (async () => {
            ...
            await page.goto('https://www.tiktok.com/@t250816121702/video/'+id);


            await page.waitForSelector('[class="jsx-1978767526 user-username"]');
            const textContent = await page.evaluate(() => document.querySelector('[class="jsx-1978767526 user-username"]').textContent);
            console.log('User Name: ' + textContent);
